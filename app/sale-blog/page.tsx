// /app/sale-blog/page.tsx (最終版 - searchParams await対応)

// 'use client' は削除済みで、Server Componentとして動作します

// ★ 修正済み: WordPress APIからデータを取得するための関数をインポート ★
...
// ★ 修正: searchParamsをawaitし、エラーを解消する ★
// searchParamsがPromiseとして扱われる場合に備え、awaitする
...
// getSalePostsに渡すパラメータ
let filterParams: { category?: string; tag?: string } = {}; 

// 1. カテゴリIDまたはタグIDに基づいてタイトルを決定し、フィルタリングパラメータを設定
if (categoryId) {
// カテゴリでフィルタリングする場合
try {
// WordPress APIからカテゴリ名を取得
