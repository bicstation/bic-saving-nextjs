...
import { AffiliateCard } from "@/components/AffiliateCard"; // 削除せず残す
import LinkCard from '../../components/LinkCard'; 

import { fetchOGPData } from "@/lib/linkParser"; 
// LinkCard from '../../components/LinkCard'; // 重複インポートの可能性あり
...
export async function generateMetadata({ params }: PostDetailPageProps): Promise<Metadata> {
    ...
    description = description
        ...
        .trim()
        .substring(0, 160) // 160文字程度にトリミング
        .replace(/(\r\n|\n|\r)/gm, ' '); // 改行をスペースに置換

    const pageUrl = `https://www.bic-saving.com/sale-blog/${slug}`;
    const imageUrl = getFeaturedImageUrl(post) || 'https://www.bic-saving.com/default-ogp-image.jpg'; // デフォルト画像をフォールバックとして設定

    // ★修正1: post オブジェクトの型を一時的に拡張して modified プロパティにアクセス可能にする
    ...
    return {
        ...
        // 3. Canonical URL
        metadataBase: new URL('https://www.bic-saving.com'),
        alternates: {
            canonical: pageUrl,
        },
        ...
        openGraph: {
            ...
            type: 'article',
            publishedTime: post.date as string, // 公開日
            // ★修正1適用: modified がない場合は post.date をフォールバックとして使用
            modifiedTime: (postWithModified.modified || post.date) as string, // 更新日
        },
