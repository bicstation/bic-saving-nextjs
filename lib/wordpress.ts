...
excerpt?: RenderedContent; // ★修正: excerpt プロパティを追加
...
try {
 ...
 })) as PostSummary[]; // ★ 修正: PostSummary[] 型であることを明示
} catch (error) {
...
// PostSummaryには _embedded が含まれないため、型アサーションを使用して互換性を保つ
...
// featured_media_url が PostSummary に直接設定されている場合 (getPostBySlugなどのマッピング結果)
